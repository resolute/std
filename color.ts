 ...
 */
export const fromHex = (input: string): number => {
 ...
 */
export const fromRgb = ([r, g, b]: number[]): number => (r << 16) | (g << 8) | b;

/**
 * Convert an integer to a hex color code.
 * @param input color integer
 */
export const toHex = (input: number): string => `#${(`000000${input.toString(16)}`).slice(-6)}`;

/**
 * Convert an integer to a RGB array
 * @param input color integer
 */
export const toRgb = (input: number): readonly [number, number, number] =>
 ...
 */
export const parse = (input: string | number | number[]): number => {
